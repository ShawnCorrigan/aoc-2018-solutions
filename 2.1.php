...
foreach($input_strings_array[0] as $input_string) {
  ...
  foreach (str_split($input_string) as $letter) {
    if (isset($previous_letters[$letter])) {
      $previous_letters[$letter]++;
    } else {
      $previous_letters[$letter] = 1;
    }
  }

  foreach ($previous_letters as $appearances) {
    if ($appearances === 2 && !$has_two) {
      $two++;

      $has_two = true;
    }
    
    if ($appearances === 3 && !$has_three) {
      $three ++;
