...
foreach ($input_strings_array[0] as $array_index => &$array_value) {
  $array_value = str_split($array_value);
}

foreach ($input_strings_array[0] as $array_index => $array_value) {
  foreach ($input_strings_array[0] as $second_array_value) {
    $differences = 0;

    foreach ($array_value as $index => $letter) {
      if ($second_array_value[$index] !== $letter) {
        $differences++;
      }
    }
    if ($differences === 1) {
      foreach ($array_value as $key => $letter) {
        if ($second_array_value[$key] !== $letter) {
  ...
  unset($input_strings_array[0][$array_index]);
}