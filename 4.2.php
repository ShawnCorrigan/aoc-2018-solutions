...
foreach($input_strings_array[0] as $input_string) {
    preg_match('/\[([^\]]+)\] ([a-z \#0-9]+)/i', $input_string, $matches);
...
foreach ($input_array as $activity) {
    if ($activity['activity'] == "falls asleep") {
        $last_asleep = substr($activity['time'], -2);
...
foreach($guards as $id => $guard) {
    foreach ($guard as $minute => $asleep) {
        if ($asleep > $most_asleep['asleep']) {
            $most_asleep['id'] = $id;
            $most_asleep['minute'] = $minute;
            $most_asleep['asleep'] = $asleep;
        }
    }
}

echo $most_asleep['id'] * $most_asleep['minute'];