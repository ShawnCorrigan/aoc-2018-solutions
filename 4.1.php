...
preg_match_all('/[^\n]+/', $input, $input_strings_array);

$input_array = [];

foreach($input_strings_array[0] as $input_string) {
    preg_match('/\[([0-9\- \:]+)\] ([a-z \#0-9]+)/i', $input_string, $matches);

    if ($matches[2] !== 'wakes up' && $matches[2] !== 'falls asleep') {
        $matches[2] = preg_replace('/Guard #([0-9]+) begins shift/', '\1', $matches[2]);
    }
...
usort($input_array, "date_sort");

$guards = [];
$last_guard = 0;
$last_asleep = 0;

foreach ($input_array as $activity) {
    if ($activity['activity'] == "falls asleep") {
        $last_asleep = intval(substr($activity['time'], -2));
    } elseif ($activity['activity'] === 'wakes up') {
        while ($last_asleep < substr($activity['time'], -2)) {
            if (isset($guards[$last_guard][$last_asleep])) {
                $guards[$last_guard][$last_asleep]++;
            } else {
                $guards[$last_guard][$last_asleep] = 1;
            }

            $last_asleep++;
        }
    } else {
        $last_guard = $activity['activity'];
    }
}
...
foreach ($guards as $id => $guard) {
    if (array_sum($guard) > $most_asleep['total_asleep']) {
        $most_asleep['id'] = $id;
        $most_asleep['total_asleep'] = array_sum($guard);
        $most_asleep['sleep_times'] = $guard;
    }
}
...
foreach ($most_asleep['sleep_times'] as $minute => $time_slept) {
    if ($time_slept > $most_slept_minute['time_slept']) {
        $most_slept_minute['time_slept'] = $time_slept;
        $most_slept_minute['minute'] = $minute;
    }
}

echo $most_asleep['id'] * $most_slept_minute['minute'];
